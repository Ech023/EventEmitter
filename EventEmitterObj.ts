export namespace EventEmitter {
	...
	export class EventListen {
		/** 事件存储：key=事件名，value=监听器数组 —— 对象驱动 */
		private _events: Record<string, EventInfo[]> = {};
		/** 异步事件队列：用于微任务批量执行，key=事件名，value=参数数组的数组 —— 对象驱动 */
		...
		public on(eventKey: string, callback: (...args: any[]) => void, target?: any, batchKey = ""): void {
			...
			let list = this._events[eventKey];
			if (!list) {
				list = [];
				this._events[eventKey] = list;
			}
		...
		public offBatch(batchKey: string): void {
			...
			for (const eventKey of Object.keys(this._events)) {
				const list = this._events[eventKey];
				const filtered = list.filter(item => item.batchKey !== batchKey);
				filtered.length ? (this._events[eventKey] = filtered) : delete this._events[eventKey];
			}
		}
		...
		public emit(eventKey: string, ...args: any[]): void {
			...
			const queue = this._queue[eventKey] || [];
			queue.push(args);
			this._queue[eventKey] = queue;
			if (!this._isFlushing) {
		...
		private _trigger(): void {
			...
			this._queue = {};
			for (const eventKey of Object.keys(queues)) {
				const argsList = queues[eventKey];
				const list = this._events[eventKey];
				...
				if (toRemove.size) {
					...
					if (remaining.length) {
						this._events[eventKey] = remaining;
					} else {
						delete this._events[eventKey];
					}
				}
